...
while true; do
  ...
  if ! pgrep -f "$MARKER" >/dev/null 2>&1; then
    ...
    echo "$(date) relaunching host — launch #$LAUNCHES (restart count $RESTARTS/$MAX_RESTARTS, fresh launch — marker-based reentry)" >> "$LOG"
    CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0 nohup caffeinate -i claude -p --dangerously-skip-permissions --model claude-fable-5 "You are a headless host process for the flight-tracking P0 workflow. Your ONLY job: (1) Invoke the Workflow tool with exactly {\"scriptPath\": \"$SCRIPT\"} — no resume parameter: cross-session resume does not restore the cache; the workflow is relaunch-aware via PASSED markers in docs/build-logs/. (2) Stay alive waiting for the workflow completion notification — do not exit, do not do other work, do not edit files in $REPO (the workflow agents own them). (3) When the workflow completes: write its return value and a phase-by-phase summary to $RESULT. Do not commit, do not push (the workflow's packager agent commits). If the Workflow tool errors on invocation, report the error as your final message and exit." > "$LAST_LOG" 2>&1 &
